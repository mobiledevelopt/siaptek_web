...
class RoleController extends Controller
{
  ...
  {
    $config['title'] = "Akses Halaman";
    $config['breadcrumbs'] = [
      ['url' => '#', 'title' => "Akses Halaman"],
    ];
    if ($request->ajax()) {
      $data = Role::query();
    ...
    // return view('contents.roles.index', compact('config'));
    return view("contents.roles.index")->with([
      'title' => "Akses Halaman"
    ]);
  }
  ...
  {
    //    $config['title'] = "Tambah Akses Halaman";
    //    $config['breadcrumbs'] = [
    //      ['url' => route('roles.index'), 'title' => "Akses Halaman"],
    //      ['url' => '#', 'title' => "Tambah Akses Halaman"],
    //    ];
    //    $config['form'] = (object)[
    //      'method' => 'POST',
  ...
  {
    $config['title'] = "Edit Akses Halaman";
    $config['breadcrumbs'] = [
      ['url' => route('roles.index'), 'title' => "Akses Halaman"],
      ['url' => '#', 'title' => "Edit Akses Halaman"],
    ];
    ...
    return view('contents.roles.form', compact('config', 'data', 'permissions'));
  }
  ...
  public function select2(Request $request)
  {
    ...
    $data = Role::where('name', 'LIKE', '%' . $request->q . '%')
      // ->when(request('roles') == 'dinas', function ($q) {
      //   return $q->where('id', '!=', 3);
      // })
      ->orderBy('name')
      ->skip($offset)
